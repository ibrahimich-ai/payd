...
(function () {
  ...
  //  Наценка — ПЛОСКАЯ: markup = цена × ставка% (ставка из тарифа уже
  //  учитывает срок). От срока и от первого взноса наценка НЕ зависит.
  ...
  function calcMurabahaPayment(amount, dpPercent, term, ratePercent) {
    ...
    const dp = a * dpP / 100;
    const body = a - dp;                 // финансируемое тело (цена − взнос), до наценки
    const markup = a * r / 100;          // ПЛОСКАЯ наценка от полной цены (мурабаха), без × (t/12)
    const total = body + markup;         // к выплате в рассрочку (тело + наценка)
